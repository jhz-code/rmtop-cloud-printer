...
class FeierCloudPrint extends FeierClient
{
     ...
     * @param int $printerId
     */
    function setPrinterId(int $printerConfigId){
    ...
    function printerAddlist(string $printerContent): string
    {
        ...
        return  $this->requestClient($this->printerConfigId,'Open_printerAddlist',$param);
    }
    //***接口返回值说明***
    ...
    function printerDelList(string $snlist){
        ...
        return  $this->requestClient($this->printerConfigId,'Open_printerDelList',$param);
    }
    ...
    function printerEdit($sn,$name,$phonenum): string
    {
        ...
        return  $this->requestClient($this->printerConfigId,'Open_printMsg',$param);
    }
    //***接口返回值说明***
...
//$content .= '<QR>http://www.feieyun.com</QR>';//把二维码字符串用标签套上即可自动生成二维码
    ...
    {
        $param = array(['sn'=>$sn, 'content'=>$content,'times'=>$times]);
        return  $this->requestClient($this->printerConfigId,'Open_printLabelMsg',$param);
    }
    //***接口返回值说明***
    //正确例子：{"msg":"ok","ret":0,"data":"123456789_20160823165104_1853029628","serverExecutedTime":6}
    //错误例子：{"msg":"错误信息.","ret":非零错误码,"data":null,"serverExecutedTime":5}
...
//$content .= "<TEXT x='9' y='10' font='12' w='1' h='2' r='0'>#001       五号桌      1/3</TEXT><TEXT x='80' y='80' font='12' w='2' h='2' r='0'>可乐鸡翅</TEXT><TEXT x='9' y='180' font='12' w='1' h='1' r='0'>张三先生       13800138000</TEXT>";//40mm宽度标签纸打印例子，打开注释调用标签打印接口打印
    ...
    {
        $param = array(['sn'=>$sn]);
        return  $this->requestClient($this->printerConfigId,'Open_delPrinterSqs',$param);
    }
    //***接口返回值说明***
    ...
    function checkPrintOrder(string $orderid): string
    {
        ...
        return  $this->requestClient($this->printerConfigId,'Open_queryOrderState',$param);
    }
    //正确例子：
    ...
    function findOrderInfoByDate($sn,$date): string
    {
        ...
        return  $this->requestClient($this->printerConfigId,'Open_queryOrderInfoByDate',$param);
    }
    //***接口返回值说明***
    //正确例子：
    ...
    {
        $param = array(['sn'=>$sn]);
        return  $this->requestClient($this->printerConfigId,'Open_queryPrinterStatus',$param);
    }
    //正确例子：
